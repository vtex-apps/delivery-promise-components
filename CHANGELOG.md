...
- **Soft refresh after session writes:** `UPDATE_ZIPCODE`, `UPDATE_PICKUP` (`selectPickup`), `SELECT_DELIVERY_SHIPPING_OPTION`, `RESET_FULFILLMENT_METHOD`, and `CLEAR_ZIPCODE` no longer trigger `window.location.reload()`. Instead, after `POST /api/sessions` resolves (and the new `vtex_segment` cookie carrying the updated `deliveryZonesHash` / `pickupPointsHash` is in place), the hook calls `apolloClient.reFetchObservableQueries(true)` so every `@withSegment` query on the page (productSearch, facets, search-suggestions, …) is refetched against the new segment without tearing the React tree down. `location.reload()` is kept as a guaranteed fallback when no `ApolloProvider` is mounted or the refetch throws.
